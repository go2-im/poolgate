...
- **CUT:** six install channels → **4** (Homebrew tap + GitHub Release binaries + verified `install.sh` + Docker; Scoop/winget/deb-rpm cut — Homebrew kept as the macOS/Linux primary, README-consistent, near-free via GoReleaser); cost-per-token accounting.
