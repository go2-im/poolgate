...
- **Proactive token refresh (idle-account keep-alive):** on each scheduler tick the engine also **proactively refreshes** any **`ok`/`unknown`** account whose credentials have not been written within `proactive_token_refresh` (a Go duration; default **12h**, `"0"` disables). This exercises the account's `refresh_token` on a bounded cadence **even with zero client traffic**, so an idle pooled account cannot silently lapse from disuse and become permanently unusable. It runs through the SAME shared single-flight refresher + version-CAS commit (so it coalesces with, and is generation-safe against, any concurrent hot-path/probe refresh), and bumps `accounts.last_refreshed_at` (a fresh account falls back to its `created_at`, so it is not refreshed on its first tick). It is deliberately bounded: **degraded (`cooldown`/`quota_exhausted`) and `expired` accounts are excluded** (they have their own re-probe cadence, and hammering a dead-token account would abuse the issuer), and the **next attempt is deferred by the interval whether the last one succeeded OR failed** (a per-account last-attempt time, not just last-success), so a persistently-failing account is retried at most once per interval — never every tick. A proactive-refresh failure is logged but does not itself transition state — a genuinely-dead token is still caught by the scheduled usage-poll/auth-check via the normal 401→`expired` path.
...
- **24.7 Theme:** **single UI language + light mode** in v1. i18n framework, accessibility program, and mobile-responsive layout are **deferred** (see §0 D9).
