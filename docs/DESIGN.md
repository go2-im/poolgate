...
- **D2 — Transport: WS-first.** Codex tries a WebSocket `/responses` upgrade first (`supports_websockets:true`, warmup `response.create generate=false`, connection-scoped `previous_response_id`). v1 originally deferred WS (HTTP POST+SSE fallback only); **post-v1 (PR #46/#50) the WS upgrade IS accepted and proxied**, configurable via `server.transport` (`both` default | `http-only` | `ws-only`). HTTP POST+SSE remains the stateless fallback (full inline `input`, no `previous_response_id`). (Supersedes §14/§23.5 "POST+SSE only" and §19.1's stateful framing.)
...
**CI (`ci.yml`, on PR/push):** `go build ./...`, `go vet`, `staticcheck`, **`go test -race ./...` + coverage gate (≥80% `internal/*` — see §25)**, `govulncheck`, and the suspicious-domain lint — all green required to merge. **Frontend build (planned — D10, product-release requirement):** the frontend **lockfile IS committed** (generated against the public npm registry `https://registry.npmjs.org`; this repo-specific decision overrides the general "don't commit lockfiles" convention), and CI runs `npm ci` + `npm run build` + `git diff --exit-code internal/webui/dist` so a stale/unrebuilt embedded `dist` fails the build. (Being wired in the current fix plan; until it lands the Go CI does not rebuild/verify `internal/webui/dist`.)
