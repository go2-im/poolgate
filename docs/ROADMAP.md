...
1. `git -C $HOME/workspaces/danny-plus/poolgate/main fetch --prune && git -C .../main merge --ff-only origin/main` (get the merged monitor + routing PRs).
2. Read this file + DESIGN §0 + REVIEW §1.
3. Next up is item 1 (**Phase 4 React admin UI**) — the notifications + real-time-monitor + accounts/concurrency backends it surfaces are all merged — then 2 → 3 → 4.
