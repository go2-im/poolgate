...
3. Next up is item 1's **last Phase 4 pages** — notifications, the real-time monitor (SSE client), and settings (accounts/policies/endpoints/keys are done; their backends are all merged) — then 2 → 3 → 4. The admin UI source is in `web/` (rebuild the embedded bundle with `npm run build`).
