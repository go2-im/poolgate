...
1. **Phase 4 — React admin UI — DONE; optional refinements remain** (`web/`, Vite, embedded via `go:embed`). **Done:** embed/serve foundation + passkey **login** + **dashboard** (PR #9); nav shell + **accounts** / **policy groups** / **endpoints** / **api keys** management pages (PR #11); **notifications** (channel CRUD + send-test over `/admin/api/notify/channels`), **real-time monitor** (live SSE tail + counters + filters over `/admin/api/monitor/*`), and **settings** (revoke-all sessions, register additional passkey, `external_origin`/`rp_id`/`secure` display via `GET /admin/api/settings`) — PR #13. Every DESIGN-required Phase 4 page now ships. **Optional refinements (deferrable, not blockers):** accounts categorize/search/sort + per-account concurrency-cap editing (needs a small account-PATCH endpoint), the full client-config generator (Codex/Cursor/curl/SDK snippets + key picker), and QR/cross-device passkey polish. DESIGN §9 phase 4, §11, §13, §15, §24.3–24.5. Single UI language + dark mode only (i18n framework/a11y/mobile deferred). Build note: the committed bundle is produced with `cd web && npm install && npm run build` (see `web/README.md`).
...
3. **Phase 4 admin UI is fully shipped** (login, dashboard, accounts/policies/endpoints/keys, notifications, monitor, settings — PRs #9/#11/#13). Next up is **item 2 — Phase 5 release pipeline** (`release.yml` + GoReleaser: signed cross-compiled binaries, cosign, Homebrew tap, Docker image), then **3** (Ops/DR & hardening) → **4** (interactive OAuth PKCE login). Optional Phase 4 refinements (accounts search/sort + concurrency-cap editing, client-config generator, QR passkey polish) can be picked up anytime but do not block 2–4. The admin UI source is in `web/` (rebuild the embedded bundle with `npm run build`).
