...
**Packages (18, all ≥80% unit-test coverage, total ~91%):** `model config crypto store oauth gateway authimport usage policy health adminauth webauthnsvc admin notify monitor webui backup` + `cmd/poolgate`.
...
1. **Ops/DR & security hardening** — **`backup`/`restore` DONE (PR #20):** `poolgate backup`/`restore` — a portable passphrase-wrapped bundle (argon2id + secretbox) carrying the master key + a consistent `VACUUM INTO` DB snapshot; read-only source (never mutates), atomic restore (temp+fsync+rename, `--force` to overwrite, env-source skips the keyfile), pre-auth KDF-param DoS cap. **Remaining:** schema-version downgrade guard + pre-migration snapshot, graceful-shutdown SSE drain, single-instance lock, clock-alignment to upstream usage timestamps, Docker hardening (read-only rootfs — the image is already distroless nonroot), docker-compose + Caddy/nginx recipes, full env-var override (`_FILE` secrets; `POOLGATE_PROXY_HOST`/`PORT` already added in PR #15), memory hygiene (disable core dumps, mlock master key), proxy-key lifecycle (expiry/rotation/IP-allowlist), audit-log completeness (append-only). DESIGN §20, §21, §22.
