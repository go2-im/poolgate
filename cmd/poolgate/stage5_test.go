...
func TestBootstrapFirstPasskeyEndToEnd(t *testing.T) {
	...
	handler, err := buildAdminHandler(cfg, st, logger, nil, nil, nil)
	if err != nil {
...
func TestBuildAdminHandlerError(t *testing.T) {
	...
	defer st.Close()
	if _, err := buildAdminHandler(cfg, st, logger, nil, nil, nil); err == nil {
...
func TestServeBothSmoke(t *testing.T) {
	...
	adminHandler, err := buildAdminHandler(cfg, st, logger, nil, nil, nil)
	if err != nil {
		t.Fatalf("buildAdminHandler: %v", err)
	}
